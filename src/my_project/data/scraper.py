...
def parse_detail_page(html: str) -> Dict[str, Any]:
    ...
    for label in soup.select(".sc-1u2rf9n, .object-params__item, .offer__param"):
        text_raw = normalize_spaces(label.get_text(" ", strip=True))
        if text_raw.lower().startswith("дом"):
            material_match = re.search(r"(монолит(?:\+кирпич)?|кирпич|панель|блок|дерево)", text_raw.lower())
            if material_match:
                building_material = material_match.group(1)
            break
